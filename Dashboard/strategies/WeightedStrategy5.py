...
class WeightedStrategy5(bt.Strategy):
    #Initialise moving average times
    params = (('fast',8), ('slow', 21), ('order_percentage', 0.95), ('ticker', 'DOT'))
    #fast-12, slow-26
    ...
    def __init__(self):
        ...
        self.rsi = bt.talib.RSI(self.data, period=8)
        
    def next(self):
        #We own 0 tokens
        if self.position.size == 0:
            #If MA cross has happened execute a buy order
            if ((self.crossover > 0) or (self.crossover2 > 0)) and ((self.rsi > 60) or (self.stoc > 60)):
        ...
        if self.position.size > 0:
            ...
            if ((self.crossover < 0) or (self.crossover2 < 0)) and ((self.rsi < 40) or (self.stoc < 40)):
